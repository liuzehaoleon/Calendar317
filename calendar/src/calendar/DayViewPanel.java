...
public class DayViewPanel extends JPanel {
	...
	public DayViewPanel() {
		...
		for (Event event : Calendar.events) {
			if (event.getDate().equals(SimpleCalendar.month.date)) {
		...
		for (int hour = 0; hour < 24; hour++) {
			...
			for (Event event : eventsOnDay) {
				if (event.getTime().getHour() == hour) {
					labels[hour].setText(event.getTitle());
			...
			c.gridy = hour;
			c.gridwidth = 2;
			gap.setPreferredSize(new Dimension(200, 25));
			panel.add(gap, c);
		}
	...
	public void update(SelectDays month) {
		...
		for (Event event : Calendar.events) {
			if (event.getDate().equals(month.date)) {
				eventsOnDay.add(event);
				System.out.println(event.getTitle());
			}
		}
